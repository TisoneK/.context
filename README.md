...
| [`core/bin/context-sync.ps1`](core/bin/context-sync.ps1) | PowerShell port for **Windows** agents (no POSIX shell): the session commands `status` / `verify` / `update` / `rollback` / `lock`. Shares `MANIFEST.sha256` with the sh tool (identical hashes). `manifest` / `bootstrap` / `harvest` stay sh-only. A `.cmd` launcher sits beside every `.ps1` port and runs it with `-ExecutionPolicy Bypass`, so Windows agents need no policy setup. |
| [`core/bin/context-collab`](core/bin/context-collab) + [`context-collab.ps1`](core/bin/context-collab.ps1) | POSIX/PowerShell helpers for opt-in peer collaboration: atomically emit immutable events, inspect live overlap, and run the integration-readiness `check` gate. |
| [`core/bin/context-gates`](core/bin/context-gates) + [`context-gates.ps1`](core/bin/context-gates.ps1) | Explicit lifecycle gates: per-turn checkpoints, pre-commit, integration, and exit commands using project-owned `memory/workflows/gates.conf`. |
