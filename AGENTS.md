...
Refreshed on core updates (fill context-ledger again). This is the canonical
...
6. **Know which kind of file you're in.** *Append-only* logs
   ...
   arranged as priority-grouped tables: add each open item
   as a row in its priority table (High/Medium/Low, `ID | Summary`),
   delete the row when its item is finished (the completion
   record is the session entry + commit, not a tombstone); `ledger-mem
   closeout` sweeps checked-off tombstones from legacy checkbox-format
   backlogs.
