...
`group-<NNN>.md`, produced by `ledger-history close`. Each file is a
...
`.context_ledger/memory/`. This zone is for audit and lookback only — a human or an
...
`ledger-history` keeps the most recent `history_keep` groups here (default
