...
**Target:** core 0.13.0 (MINOR — additive zones + a new `ledger-history`
...
- **Durable, cross-group (stays in `memory/`, never rotates):**
  ...
  (update-in-place, and `ledger-mem prune`/archive from 0.12.0).
...
`history/` and `archive/` are new top-level zones under `.context_ledger/`, added by
...
| **archive forced-delete** | When `archive/` exceeds **`archive_keep`** (default **12**) tarballs, **oldest-first** deletion down to the cap. Forced, no exemptions. Runs only via the explicit `ledger-history gc` subcommand. | The spec demands a hard cap and consistency (no ad-hoc exemptions). Isolating deletion in `gc` keeps the one destructive step deliberate. |
...
`core/bin/ledger-history` (POSIX) + `ledger-history.ps1`:
...
grouped `agents/sessions.md` mode; `ledger-sync manifest` is regenerated.
