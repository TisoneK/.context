# Universal Agent Kickoff — `.context_ledger/` Protocol Entry Point (Bootstrap)

> **Hand this file to the agent for a project's FIRST-EVER `.context_ledger/`
...
> project as `.context_ledger/core/`, create the project's memory in
> `.context_ledger/memory/`, generate the in-repo entry points, push. After
...
> the project repo already contains `.context_ledger/kickoff.md`, switch to it
> now and follow it. If it contains `.context_ledger/` but no `kickoff.md` (or
...
> completes, a fresh copy exists at `../context-ledger/universal-kickoff.md` —
...
>
> ```bash
> diff <this-file> ../context-ledger/universal-kickoff.md >/dev/null \
>   && echo "copy is current — proceed" \
>   || echo "copy is STALE — follow ../context-ledger/universal-kickoff.md from Step 1 on"
> ```
...
codebase and its `.context_ledger/` memory in a better state.
...
- **Package repo URL:** https://github.com/TisoneK/context-ledger.git _[default — change if you use a fork/mirror]_
- **Is the package repo private?** <Yes / No> _[default **No** — the canonical `TisoneK/context-ledger` is **public** (renamed + made public 2026-09-08). If you use a private fork/mirror, answer Yes; the clone commands in Step 0 handle either case.]_
...
> tree INTO the project as `.context_ledger/core/` — versioned, checksummed,
> read-only — beside the project's own writable memory in
> `.context_ledger/memory/`:

| | Lives at | Owner | Sync |
|---|---|---|---|
| **Vendored protocol** | `<REPO>/.context_ledger/core/` | package | whole-tree, via `ledger-sync update` |
| **Project memory** | `<REPO>/.context_ledger/memory/` | project | never synced — yours forever |

After the bootstrap push, every future session — any agent type, any
machine — starts from `.context_ledger/kickoff.md` inside the repo and runs
entirely from the vendored core. The package repo is touched again only
to **update** core (`ledger-sync status/update`, any later session,
...
local clones exist under different names (`../context-ledger` is
canonical; legacy `../context` and `../.context` occur):

```bash
PKG=""
for d in ../context-ledger ../context ../.context; do
  git -C "$d" remote get-url origin 2>/dev/null | grep -q "TisoneK/context-ledger" \
    && PKG="$d" && break
done
...
if [ -n "$PKG" ]; then
  ...
  git clone https://github.com/TisoneK/context-ledger.git ../context-ledger && PKG=../context-ledger
fi
...
```bash
cd <workspace>
git clone https://github.com/TisoneK/context-ledger.git context-ledger   # public (default)
# private fork/mirror: git clone "https://x-access-token:${PKG_TOKEN}@github.com/<PKG_OWNER>/<PKG_REPO>.git" context-ledger
git -C context remote set-url origin https://github.com/TisoneK/context-ledger.git
...
```bash
ls "$PKG"/core 2>/dev/null || ls ../context-ledger/core    # VERSION, rules/, schemas/, templates/, bin/
```

Diff the copy of this file you were handed against
`../context-ledger/universal-kickoff.md` (see the bootloader note at the top)
— if they differ, execute Steps 1–4 from the **package's** copy.

> **Two directories, don't conflate them:** `../context-ledger` (a
> sibling) is the package clone — this session's source; `./.context_ledger`
> (inside the project) is what you're about to create. After this session
> only the second one matters.

---

## Step 1 — Bootstrap `.context_ledger/` (Path A) or Hand Over (Path B)

### Path B first: `.context_ledger/` already exists

- Has `.context_ledger/kickoff.md` → **you're in the wrong file**: read
  `.context_ledger/kickoff.md` and follow it instead. Done here.
- Has `.context_ledger/` but flat (no `core/`+`memory/` zones) → **migration**,
  not bootstrap: follow `../context-ledger/MIGRATION.md`.

### Path A: `.context_ledger/` does not exist — bootstrap it

#### 1a. Run the bootstrapper

```bash
sh ../context-ledger/core/bin/ledger-sync bootstrap .
```
...
It vendors `core/` → `.context_ledger/core/`, copies the memory skeleton →
...
in) + `.context_ledger/.gitattributes` (LF policy), and writes `memory/core.lock`.
Verify (Windows: `.context_ledger/core/bin/ledger-sync.cmd verify`):
...
# ANY output from the second line = a nested/double copy — rm -rf .context_ledger and redo 1a.
```
...
- **`.context_ledger/memory/user/identity.md`** — name, git identity, GitHub username, role, timezone
- **`.context_ledger/memory/user/preferences.md`** — seeded from Pre-Flight session parameters
...
- **`.context_ledger/kickoff.md`** — fill Project Facts per its HTML-comment
...
- Placeholder scan (covers 1b too — run before committing):
  ```bash
  grep -rn "<PROJECT_NAME>\|<PROJECT_REPO_URL>\|<GIT_NAME>\|<GIT_EMAIL>\|<LIVE_URL" .context_ledger/ AGENTS.md CLAUDE.md
...
```bash
git add .context_ledger/ AGENTS.md CLAUDE.md
git commit -m "chore(ledger): bootstrap .context_ledger/ (core $(cat .context_ledger/core/VERSION))

Vendored protocol core + initial memory from TisoneK/context-ledger.
Entry point for future sessions: .context_ledger/kickoff.md"
...
## Step 2 — Read `.context_ledger/`, Step 3 — Load the Protocol, Step 4 — Follow It

From here the generated `.context_ledger/kickoff.md` — whose Entry Steps are
the canonical version of what follows — takes over:

1. **Read** `.context_ledger/README.md` (zone map), then the memory files in
   its listed order (sessions, tasks, logs, decisions, overrides,
   system, user).
2. **Load your edition from the vendored core, by YOUR agent type:**
   local → `.context_ledger/core/rules/ai-engineering-protocol-local.md`;
   cloud/sandbox → `.context_ledger/core/rules/ai-engineering-protocol.md`.
   Plus any role overlay from `.context_ledger/core/roles/`. Read it in full —
...
- **Don't start the project's server** unless the task requires it.
  "Start ledger workflow" means follow this protocol, not run the app.
- **Don't grep the codebase for "ledger"** to find the protocol — read
  `.context_ledger/` directly.
- **Don't write anything under `.context_ledger/core/`** — it's the vendored,
  checksummed protocol. Memory goes under `.context_ledger/memory/`.
...
- **Don't carry this file to the next session** — the project now has
  `.context_ledger/kickoff.md`; hand THAT to the next agent.
...
| If you need... | Look in... |
|---|---|
| The entry point for every future session | `<REPO>/.context_ledger/kickoff.md` (supersedes this file) |
| The protocol editions | `<REPO>/.context_ledger/core/rules/` |
...
real entry points: `.context_ledger/kickoff.md` (the front door), `AGENTS.md`
...
miss the protocol). From the next session on, *"Read `.context_ledger/kickoff.md`
