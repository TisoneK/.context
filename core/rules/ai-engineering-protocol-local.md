...
- If the working tree is dirty (uncommitted changes), STOP and report: "Working tree has uncommitted changes. Please commit or stash them before I start." When you do, check `.context_ledger/memory/agents/roster.md` and any collaboration events first — the changes may be a teammate's claim in progress. Attribute them; don't investigate or touch them.
...
- If `.context_ledger/` exists, read it in this order:
  ...
  2. `.context_ledger/memory/agents/sessions.md` — who worked here before, with which model, on which machine, and what they did (read the last 3–5 entries)
...
- If this session completed an existing backlog item, **delete its line** from `backlog.md` — the backlog holds only undone or partially done work. The completion record is this session's `agents/sessions.md` entry and the commit itself; git history preserves the removed line. Never delete a line whose item is still open.
...
└── memory/              # ZONE 2 — this project's living memory: project-owned, writable
    ...
    │   └── backlog.md       # live queue of open items — delete a line when its item is done
    ├── plans/
...
5. **Learn from it**: log it in `.context_ledger/memory/inefficiencies/log.md` if it reveals a workflow gap.
...
- If a command fails or a tool is missing, tell the user plainly. Don't silently work around it — and log it in `.context_ledger/memory/inefficiencies/log.md`.
...
23. **Don't edit append-only logs** — `sessions.md`, `inefficiencies/log.md`, `decisions.md` grow by appending. Wrong entries get appended corrections, not deletions. (`backlog.md` is not in this set — it's a live queue: delete a line when its item is done.)
...
39. **Don't commit an append-only file whose diff shows removed lines** — before committing `agents/sessions.md`, `plans/decisions.md`, `inefficiencies/log.md`, or `flaws/log.md`, run `git diff <file>` and confirm every changed line is a `+`. A `-` line means you edited or overwrote history: restore the file and re-append instead. (Sole exception: the documented byte-identical-duplicate removal, which leaves a note in place. `tasks/backlog.md` is exempt — it's a live queue, so `-` lines are expected when an item is finished.)
...
- **Build failing?** Run the typecheck command first. Check if a recent commit broke something. The user's IDE may already be showing the errors. Check `.context_ledger/memory/inefficiencies/log.md` — a prior agent may have hit and solved this exact failure.
...
> **The session record** goes in `.context_ledger/memory/agents/sessions.md` — it travels with the repo, so the cloud agent and the local agent (you) share one memory.
