...
- **One protocol package repo** — `TisoneK/.context`, cloned at
  `~/Code/context`. Its [`core/`](core/) tree is the product: editions,
  roles, schemas, templates, the `context-sync` tool. You improve it
  once, over time.
- **Each project carries `.context/`** with two zones: `core/` (a
...
│   ├── rules/ · roles/ · schemas/ · templates/ · bin/context-sync
...
The package repo is **private** — your GitHub account needs access.
...
sh ~/Code/context/core/bin/context-sync bootstrap .
...
> Tell the agent: **"Read `.context/kickoff.md` and follow it."**
> Add a target in the same message if you have one.

That file routes the agent by its own type:

```text
read .context/README.md (zones)  →  context-sync verify/status (core health)
→  read memory/ (sessions, tasks, logs, decisions, overrides, prefs)
→  load .context/core/rules/<your-type's edition> (+ role overlay)
→  work  →  update memory/  →  push
```

- Memory updates commit as `chore(context):`; reports as `docs(review):`.
...
```bash
sh .context/core/bin/context-sync status    # drift check at session start
...
Projects bootstrapped before 0.2.0 (flat `.context/`, sibling-clone
protocol): see [`MIGRATION.md`](MIGRATION.md).

## How the package and a project divide the work

| | Package repo (`TisoneK/.context`) | Project's `.context/` |
|---|---|---|
